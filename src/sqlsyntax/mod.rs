...
mod test {
    ...
    fn create_table_parsing_test() {
        let create_table = parse("CREATE TABLE test (
        ...
        match create_table.unwrap() {
            Statement::Create(CreateStatement::Table(CreateTableStatement { table, columns })) => {
                assert_eq!(table, Table { database_name: None, table_name: "test".to_string() });
                assert_eq!(columns, vec![
                    CreateTableColumn {
                        ...
                        constraints: vec![
                            CreateTableColumnConstraint { name: Some("pk".to_string()), constraint: PrimaryKey }
                        ]
                    },
                    CreateTableColumn {
                    ...
                    CreateTableColumn {
                        ...
                        constraints: vec![
                            CreateTableColumnConstraint { name: None, constraint: Nullable },
                            CreateTableColumnConstraint { name: None, constraint: Unique }
                        ]
                    }
                ]);
            }
    ...
    fn insert_parsing_test() {
        ...
        match insert_sql1.unwrap() {
            Statement::Insert(InsertStatement { table, into_columns, source }) => {
                assert_eq!(table, Table {
                    database_name: None,
                    table_name: "table1".to_string(),
                });
                assert_eq!(into_columns, None);
                assert_eq!(source, InsertSource::Values(vec![
        ...
        match insert_sql2.unwrap() {
            Statement::Insert(InsertStatement { table, into_columns, source }) => {
                ...
                assert_eq!(source, InsertSource::Values(vec![vec![
                    BinaryOp { lhs: Box::new(StringLiteral("foo".to_string())), rhs: Box::new(StringLiteral("bar".to_string())), op: Concatenate }, Number("2".to_string())
                ]]));
            },
            st => { panic!("Expected insert but actually={:?}", st) }
        }
        ...
        match insert_sql3.unwrap() {
            Statement::Insert(InsertStatement { table, into_columns, source }) => {
                ...
                assert_eq!(into_columns, None);
                assert_eq!(source, InsertSource::Select(Box::new(SelectStatement {
    ...
    fn select_parsing_test() {
        ...
        match select_sql1.unwrap() {
            Statement::Select(SelectStatement { result_columns, from, where_expr, group_by, having, order_by }) => {
                assert_eq!(result_columns, vec![
                    AllColumns,
                    Expr {
                        expr: BinaryOp {
                ...
                assert_eq!(having, Some(BinaryOp {
                    lhs: Box::new(FunctionCallAggregateAll { name: "count".to_string() }),
        ...
        let select_sql2 = parse("SELECT * FROM foo INNER JOIN bar ON foo.id = bar.fooId ORDER BY a DESC, b;");
        match select_sql2.unwrap() {
            Statement::Select(SelectStatement { result_columns, from, where_expr, group_by, having, order_by }) => {
                ...
                assert_eq!(order_by, vec![
                    OrderingTerm { expr: Ident("a".to_string()), order: Descending },
                    OrderingTerm { expr: Ident("b".to_string()), order: Ascending }
                ]);
            },
            st => { panic!("Expected select statement but actually={:?}", st) }
        }
        ...
        match select_sql3.unwrap() {
            Statement::Select(SelectStatement { result_columns, from, where_expr, group_by, having, order_by }) => {
                ...
                assert_eq!(from, Cross(vec![TableOrSubquery::Table { table: Table { database_name: None, table_name: "track".to_string() }, alias: None }]));
                assert_eq!(where_expr, None);
                assert_eq!(group_by, Vec::new());
                assert_eq!(having, None);
                assert_eq!(order_by, Vec::new());
    ...
    fn explain_parsing_test() {
        let explain_sql = parse("EXPLAIN select avg(milliseconds) / 1000 seconds FROM track where name = 'test';");
        match explain_sql.unwrap() {
            Statement::Explain(Select(SelectStatement { result_columns, from, where_expr, group_by, having, order_by })) => {
                assert_eq!(result_columns, vec![
                    Expr {
                        expr: BinaryOp {
                            lhs: Box::new(FunctionCall {
                ...
                assert_eq!(from, Cross(vec![
                    TableOrSubquery::Table { table: Table { database_name: None, table_name: "track".to_string() }, alias: None }
                ]));
                assert_eq!(where_expr, Some(BinaryOp { lhs: Box::new(Ident("name".to_string())), rhs: Box::new(StringLiteral("test".to_string())), op: Equal }));
                assert_eq!(group_by, Vec::new());
                assert_eq!(having, None);
                assert_eq!(order_by, Vec::new());
            },
    ...
    fn parsing_errors_test() {
        match parse("") {
            Err(err) => {
                assert_eq!(err, ExpectingFirst("SELECT, INSERT, CREATE, or EXPLAIN statement", None))
            },
            st => { panic!("Expected error but actually={:?}", st) }
        }
        match parse("123") {
            Err(err) => {
                assert_eq!(err, ExpectingFirst("SELECT, INSERT, CREATE, or EXPLAIN statement", Some(Token::Number("123".to_string()))))
            },
            st => { panic!("Expected error but actually={:?}", st) }
        }
        match parse("select select from USERS") {
            Err(err) => {
                assert_eq!(err, Expecting("* or expression for SELECT column", Some(Token::Select)))
            },
            st => { panic!("Expected error but actually={:?}", st) }
        }
